...
namespace FOW
{
    ...
    public class FogOfWarWorld : MonoBehaviour
    {
        ...
        [Tooltip("The color of the fog")] public Color UnknownColor = new Color(.35f, .35f, .35f);
        ...
        [Range(0, 2)] public float BlurDistanceScreenPercentMin = .1f;
        [Range(0, 2)] public float BlurDistanceScreenPercentMax = 1;
        public int BlurSamples = 6;

        public Texture2D FogTexture;
        public Texture2D FogTextureRegrow;
        ...
        //delite
        void SetPixelsColors(RenderTexture rt, Color color, Vector2Int[] positions)
        {
            // Створення текстурного об'єкту на основі RenderTexture
            Texture2D tex = new Texture2D(rt.width, rt.height, TextureFormat.RGBA32, false);

            // Читання даних кольорів з RenderTexture у текстурний об'єкт
            RenderTexture.active = rt;
            tex.ReadPixels(new Rect(0, 0, rt.width, rt.height), 0, 0);
            tex.Apply();

            // Встановлення кольорів для певних пікселів в текстурному об'єкті
            ...
            // Застосування змін до текстурного об'єкту
            tex.Apply();

            // Копіювання даних з текстурного об'єкту назад у RenderTexture
        ...
        public void Added(Vector2Int[] shapePoints)
        {
            var changedPixel = PixelTools.FillShape(
                FOW_RT.height,
                shapePoints);

            SetPixelsColors(FOW_RT, Color.clear, changedPixel.Select(pixel => new Vector2Int(pixel.Key, pixel.Value)).ToArray());
            RenderTexture.active = FOW_REGROW_RT;
            FogTextureRegrow.ReadPixels(new Rect(0, 0, FOW_REGROW_RT.width, FOW_REGROW_RT.height), 0, 0);
            FogTextureRegrow.Apply();

        }

        public void Added()
        {
            var shapePoints = new Vector2Int[20];
            for (int i = 0; i < shapePoints.Length; i++)
            {
                var vec = GeometryUtils.VectorTransformationWithRotation(
                    (360 / 20) * i,
                    Vector3.forward,
                    100);
                shapePoints[i] = Vector2Int.zero + new Vector2Int((int)vec.x, (int)vec.z);
            }
        ...
        public void InitFOWRT()
        {
            ...
            if (UseRegrow)
            {
                ...
                FogTextureRegrow = new Texture2D(FOW_REGROW_RT.width, FOW_REGROW_RT.height, TextureFormat.RGBA32, false);
                FogTextureRegrow.Apply();
            }
        }
        ...
        public void SetFogShader()
        {
            ...
            switch (FogAppearance)
            {
                ...
                case FogOfWarAppearance.Texture_Sample:
                    shaderName += "/TextureSample";
                    break;
                case FogOfWarAppearance.Outline:
                    shaderName += "/Outline";
                    break;
                case FogOfWarAppearance.None:
        ...
        public void UpdateMaterialProperties(Material material)
        {
            ...
            switch (FogType)
            {
                case FogOfWarType.Hard:
                    material.EnableKeyword("HARD");
                    break;
                case FogOfWarType.Soft:
        ...
        public bool CheckVisibility(Vector2Int NumPixel)
        {
            var color = FogTextureRegrow.GetPixel(NumPixel.x, NumPixel.y);
        ...
        {
            [ReadOnly] public NativeArray<ConeEdgeStruct> Angles;
            [WriteOnly] public NativeArray<ConeEdgeStruct> AnglesArray;
